...
boru-driven command-line client **and** interactive REPL for the Punkapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/punkapi-cli)
make build
...
./punkapi-cli list beer
./punkapi-cli load 1 beer            # {id:1} shorthand
./punkapi-cli load '{id:1}' beer       # explicit match map
./punkapi-cli list image

# 5. Override the API base URL for a single call
PUNKAPI_BASE=https://api.example.com ./punkapi-cli list beer
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/punkapi-cli
   ```
...
```sh
./punkapi-cli load 1 beer          # scalar shorthand for {id:1}
./punkapi-cli load '{id:1}' beer     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `punkapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `beer`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/punkapi-cli`. |
...
(`./punkapi-cli <expr>`) and interactively (the REPL), and expressions compose the
