// Command Punkapi-cli is a CLI + REPL for the Punkapi SDK,
...
const prompt = "punkapi"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "beer image"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PUNKAPI_APIKEY (api key), PUNKAPI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PUNKAPI_APIKEY carries the API key and
	// PUNKAPI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PUNKAPI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PUNKAPI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPunkapiSDK(opts)
