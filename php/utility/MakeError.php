...
class PunkapiMakeError
{
    public static function call(?PunkapiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PunkapiContext([], null);
        }
        $op = $ctx->op ?? new PunkapiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PunkapiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PunkapiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PunkapiError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof PunkapiError) {
