...
class PunkapiSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PunkapiHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PunkapiHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PunkapiFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PunkapiFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PunkapiUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PunkapiHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PunkapiHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PunkapiHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PunkapiError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PunkapiHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PunkapiError("graphql_error",
                "PunkapiSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Beer($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_beer === null) {
                $this->_beer = new BeerEntity($this, null);
            }
            return $this->_beer;
        }
        return new BeerEntity($this, $data);
    }
