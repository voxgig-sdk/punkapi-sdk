...
The PHP SDK for the Punkapi API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Beer()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($beers as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["abv"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $beer));
```
...
```
php/
├── punkapi_sdk.php          -- Main SDK class
...
The main class (`punkapi_sdk.php`) exports the SDK class
