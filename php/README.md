...
The PHP SDK for the Punkapi API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Beer(null)->list(null, null);
...
```php
[$result, $err] = $client->Beer(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->Punkapi(null)->load(
    ["id" => "test01"], null
);
