...
[MCP](https://modelcontextprotocol.io) server exposing the Punkapi SDK as
two agent tools — `punkapi_list` and `punkapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/punkapi-mcp)
make build

# 2. Provide credentials via the environment
export PUNKAPI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user punkapi \
  -- /absolute/path/to/punkapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./punkapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// punkapi_list: first page of records
{ "entity": "beer" }
{ "entity": "beer", "query": { } }

// punkapi_load: one record by id
{ "entity": "beer", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/punkapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user punkapi \
     -- "$PWD"/dist/*/punkapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `punkapi_list` and `punkapi_load` tools now appear
   in new sessions. Ask the agent to *"list beer using punkapi"*
   and it calls `punkapi_list` with `{"entity":"beer"}`.
...
```sh
export PUNKAPI_APIKEY=sk_live_xxx            # API key
export PUNKAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `punkapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `punkapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PUNKAPI_APIKEY` | API key sent with every request. |
| `PUNKAPI_BASE` | Optional override of the API base URL. |
...
```sh
./punkapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"punkapi_load","arguments":{"entity":"beer","query":{"id":1}}}}'
```
