...
MCP server exposing the Punkapi SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `punkapi_list` | `entity`, optional `query` map | First page of records as JSON |
| `punkapi_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `punkapi_list` and
`punkapi_load` tools become available in new sessions.
