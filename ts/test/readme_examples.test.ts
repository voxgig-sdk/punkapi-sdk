...
//     (\`new PunkapiSDK(...)\` / \`PunkapiSDK.test(...)\` become
//     \`PunkapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"beer":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PunkapiSDK(...)\` and any
// \`PunkapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PunkapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PunkapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
