...
The Golang SDK for the Punkapi API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.


## Install
```bash
go get github.com/voxgig-sdk/punkapi-sdk
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/punkapi-sdk=../path/to/github.com/voxgig-sdk/punkapi-sdk
```
...
```go
    result, err := client.Beer(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
...
| `Image` | `(data map[string]any) PunkapiEntity` | Create a Image entity instance. |
...
```
github.com/voxgig-sdk/punkapi-sdk/
...
The root package (`github.com/voxgig-sdk/punkapi-sdk`) re-exports everything needed
