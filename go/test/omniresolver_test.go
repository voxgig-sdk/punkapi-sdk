...
import (
	...
	omni "github.com/voxgig-sdk/punkapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PunkapiError); is {
				out := map[string]any{
					"name":    "PunkapiError",
...
func omniCtx(arg any, client *sdk.PunkapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PunkapiSDK); is {
			client = live
