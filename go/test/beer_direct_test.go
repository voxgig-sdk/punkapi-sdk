...
func TestBeerDirect(t *testing.T) {
	t.Run("direct-list-beer", func(t *testing.T) {
		setup := beerDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-beer", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-beer", func(t *testing.T) {
		setup := beerDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-beer", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type beerDirectSetupResult struct {
	client *sdk.PunkapiSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func beerDirectSetup(mockres any) *beerDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PUNKAPI_TEST_BEER_ENTID"]; ok {
		...
		return &beerDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &beerDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
