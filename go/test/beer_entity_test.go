...
func TestBeerEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Beer(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Beer(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		beerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.beer")))
		...
		beerRef01DataDt0LoadResult := core.ToMapAny(entityData(beerRef01DataDt0Loaded))
