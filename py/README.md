...
The Python SDK for the Punkapi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Beer(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Beer(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Punkapi(None).load(
    {"id": "test01"}, None
)
