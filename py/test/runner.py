# Punkapi SDK test runner

from __future__ import annotations
import os
import json

from punkapi_sdk.utility.voxgig_struct import voxgig_struct as vs


class PunkapiTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PunkapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PunkapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PunkapiTestRunner.getenv("PUNKAPI_TEST_LIVE")
        override = PunkapiTestRunner.getenv("PUNKAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PunkapiTestRunner.getenv(key)
        ...
        explain = PunkapiTestRunner.getenv("PUNKAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PUNKAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PunkapiTestRunner._test_control is not None:
            return PunkapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PunkapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PunkapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PunkapiTestRunner.load_test_control()
...
def load_env_local():
    PunkapiTestRunner.load_env_local()


def env_override(m):
    return PunkapiTestRunner.env_override(m)


def entity_data(v):
    return PunkapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PunkapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PunkapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PunkapiTestRunner.load_test_control()


def live_client_options():
    return PunkapiTestRunner.live_client_options()


def live_delay_ms():
    return PunkapiTestRunner.live_delay_ms()
