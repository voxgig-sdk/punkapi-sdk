...
class BeerEntity
  ...
  def make
    opts = @_entopts.dup
    BeerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PunkapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PunkapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Beer.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PunkapiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BeerListMatch, Hash, nil] match filter (any subset of
  #   Beer fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
