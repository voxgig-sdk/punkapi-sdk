...
require_relative 'Punkapi_types'
...
class PunkapiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PunkapiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PunkapiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PunkapiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PunkapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PunkapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PunkapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PunkapiError.new(
        "#{op}_allow",
        "PunkapiSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PunkapiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PunkapiError.new(
        "graphql_error", "PunkapiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Beer.list / client.Beer.load({ "id" => ... })
  def Beer(data = nil)
    require_relative 'entity/beer_entity'
    BeerEntity.new(self, data)
  end
