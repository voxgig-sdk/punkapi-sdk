...
The Lua SDK for the Punkapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Beer(nil):list(nil, nil)
...
```lua
local result, err = client:Beer(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Punkapi(nil):load(
  { id = "test01" }, nil
)
