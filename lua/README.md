...
The Lua SDK for the Punkapi API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Beer()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/punkapi-sdk/releases)),
...
| `food_pairing` | `table` | List of foods that pair well with this beer |
...
```
lua/
├── punkapi_sdk.lua    -- Main SDK module
...
The main module (`punkapi_sdk`) exports the SDK constructor
