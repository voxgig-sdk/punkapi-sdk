...
local BeerEntity = {}
BeerEntity.__index = BeerEntity


function BeerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BeerEntity)
  self._name = "beer"
  self._client = client
...
function BeerEntity:get_name()
  return self._name
end


function BeerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BeerEntity.new(self._client, opts)
end
...
function BeerEntity:mark_deleted()
  self._deleted = true
end


function BeerEntity:deleted()
  return true == self._deleted
end


function BeerEntity:data_set(args)
  if args ~= nil then
...
function BeerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BeerEntity:match_set(args)
  if args ~= nil then
...
function BeerEntity:stream(action, args, callopts)
...
---@param reqmatch BeerLoadMatch
---@param ctrl? table
---@return Beer
---@return string? err
function BeerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch BeerListMatch
---@param ctrl? table
---@return Beer[]
---@return string? err
function BeerEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function BeerEntity:_run_op(ctx, post_done)
  local utility = self._utility
