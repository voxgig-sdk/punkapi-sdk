...
PunkapiSDK._make_feature = _make_feature


function PunkapiSDK.new(options)
  local self = setmetatable({}, PunkapiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PunkapiSDK:options_map()
...
function PunkapiSDK:get_utility()
  return Utility.copy(self._utility)
end


function PunkapiSDK:get_root_ctx()
  return self._rootctx
end


function PunkapiSDK:prepare(fetchargs)
...
function PunkapiSDK:direct(fetchargs)
...
-- by passing it.
function PunkapiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PunkapiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PunkapiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PunkapiSDK:Beer(data)
  ...
  if data == nil then
    if self._beer == nil then
      self._beer = EntityMod.new(self, nil)
    end
    return self._beer
  end
...
function PunkapiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PunkapiSDK.new(sdkopts)
  sdk.mode = "test"
