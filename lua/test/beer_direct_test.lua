...
describe("BeerDirect", function()
  it("should direct-list-beer", function()
    local setup = beer_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-beer", setup.live and "live" or "unit")
  ...
  it("should direct-load-beer", function()
    local setup = beer_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-beer", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function beer_direct_setup(mockres)
