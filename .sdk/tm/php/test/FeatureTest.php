...
class FtCtrl extends PunkapiControl
{
...
class FtClient
{
    ...
    private PunkapiUtility $utility;

    public function __construct(PunkapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): PunkapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PunkapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PunkapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PunkapiConfig::make_feature($name);
    ...
    private static function buildUrl(PunkapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PunkapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PunkapiResponse($fetched);
        }
    ...
    private function fail(PunkapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PunkapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
